...
Ext.tree.ColumnTree = Ext.extend(Ext.tree.TreePanel, {
    ...
    onRender: function() {
        ...
        var tabWidth = Ext.getBody().getSize().width-170;
        ...
        for (var i = 0, len = cols.length; i < len; i++) {
            ...
            this.headers.createChild({
                ...
                style: 'width:' + ((c.width/totalWidth)*tabWidth - this.borderWidth) + 'px;'
            });
        }
        this.headers.createChild({
            cls: 'x-tree-hd ',
        ...
        this.headers.setWidth(tabWidth);
//        totalWidth -= this.scrollOffset;
        this.innerCt.setWidth(tabWidth);
    }
});
Ext.tree.ColumnTreeNodeUI = Ext.extend(Ext.tree.TreeNodeUI, {
    ...
    renderElements: function(n, a, targetNode, bulkRender) {
        ...
        var buf = ['<li class="x-tree-node"><div ext:tree-node-id="', n.id, '" class="x-tree-node-el x-tree-node-leaf x-unselectable ', a.cls, '" unselectable="on">',
                '<div class="x-tree-col" style="width:', (c.width/totalWidth)*tabWidth - bw, 'px;">',
        ...
        for (var i = 1, len = cols.length; i < len; i++) {
            c = cols[i];
            buf.push('<div class="x-tree-col ', (c.cls ? c.cls : ''), '" style="width:', (c.width/totalWidth)*tabWidth - bw, 'px;">',
...
Ext.extend(Ext.ux.ColumnTreeCheckNodeUI, Ext.tree.ColumnTreeNodeUI, {
    ...
    check: function(checked) {
        ...
        if (checked === null) {
            ...
            if(!checked){
            	this.parentCheckSecond(n.parentNode, checked);
            }
        } else {
 ...
 // private
    //sunping add
    //系统维护列表当子节点字少有一个没选中，父节点不选中
    parentCheckSecond: function(node, checked) {
        var checkbox = node.getUI().checkbox;
        if (typeof checkbox == 'undefined') return;
        if (!(checked ^ checkbox.checked)) return;
        checkbox.checked = checked;
